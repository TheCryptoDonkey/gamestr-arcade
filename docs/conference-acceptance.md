...
`npm run conference:status` is the evidence gate for a player-facing cabinet.  Run it against the installed booth catalogue with `npm run conference:status -- /home/<booth-user>/gamestr-games`.  It remains red until every controller title has exact physical-booth evidence; desk tests and a successful package build do not clear that gate.  A green result still does not replace proving the packaged artifact and live service on the final booth.
...
4. Disconnect and reconnect the controller at the carousel and once inside the title.  Repeat over the booth's wired and wireless paths until both `map=standard` and `map=none`/Linux HAT profiles have been observed.  Controller telemetry is off during normal sessions; temporarily set `ARCADE_GAMEPAD_DIAGNOSTICS=1` in a systemd override and restart the service while collecting this evidence, then remove the override.
...
- Run once with conference networking disabled.  Confirm the shell remains recoverable, set `conferenceFallback: true` only on the proven local title, and verify its `exec` file or `site/index.html` is actually installed.  A remote URL plus `network: optional` is not offline evidence.
- Run `npm run check` on the exact checkout used to package the booth, then run `npm run conference:status -- /home/<booth-user>/gamestr-games` against the installed catalogue.
